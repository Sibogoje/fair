            ...
            <div class="card-body">
			  ...
			  <form class="row g-3 needs-validation" id="user_form" method="post" action="consolcsv.php" target="_blank"  enctype="multipart/form-data" novalidate>
                  ...
                  <div class="form-floating">
					 ...
					 <select type="text" class="form-control" id="single"    placeholder="MemberID" name="single"  required>
					<option value="" selected></option>
						<?php 
						$stmt12 = $conn->prepare("SELECT DISTINCT * FROM `tblretirementfunds1` ");
						$stmt12->execute();
						$result12 = $stmt12->get_result();
						if ($result12->num_rows > 0) {
						  // output data of each row
						while($row12 = $result12->fetch_assoc()) {
						$MemberNo = $row12['RetirementFundID'];
						//$retirementfund = $row12['RetirementFundID'];
					    	
							

						?>
					<option value="<?php echo $row12['RetirementFundID']; ?>"><?php echo $row12['RetirementFundID']." - ".$row12['FundName']; ?></option>
						<?php   }
						} else {
						  echo "0 results";
						} ?> 
