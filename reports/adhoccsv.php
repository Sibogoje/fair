...
if (in_array("all", $mntharray)){
   ...
   $choose = "`tblMemberAccounts1` WHERE TransactionTypeID = '4' AND  DATE(TransactionDate) BETWEEN '$d1'  AND '$d2'  ORDER BY TransactionDate DESC, accountsID DESC";  
}else{
  
 $choose = "`tblMemberAccounts1` WHERE TransactionTypeID = '4' AND  `memberID` IN ({$mntharray2}) AND DATE(TransactionDate) BETWEEN '$d1'  AND '$d2'  ORDER BY TransactionDate DESC, accountsID DESC";   
}
